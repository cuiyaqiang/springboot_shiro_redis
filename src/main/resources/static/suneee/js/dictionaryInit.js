...
function dictInit() {
	...
	if (isEmptyObject(localData)) {
		$.ajax({
			...
			success : function(data) {
				if(data.returnCode == 1){// base接口数据字典获取成功才赋值
					localData = data;
				}
			}
		});
		...
		if (key == keyword) {
			$.each(objArr, function(id, obj) {
				...
				if (cloneSpan.length > 0) {
					...
					span = "<span style='margin-right:20px;'>" + obj.ddlname + "</span>";
				}
...
function apiUrlInit() {
	$("select[apiUrl]").each(function() {
		selectInitByApiUrl($(this),"apiUrl");
		$(this).removeAttr("apiUrl");
	})

	$("select[localApiUrl]").each(function() {
		selectInitByApiUrl($(this),"localApiUrl");
		$(this).removeAttr("localApiUrl");
	})
...
// 根据apiUrl初始化select
function selectInitByApiUrl(select,urlType) {
	var url;



	if(urlType=="apiUrl"){
		url = baseStrUrl+select.attr(urlType);
	}

	if(urlType=="localApiUrl"){
		url = baseUrl+select.attr(urlType);
	}
	...
	if (typeof (where) == "undefined" || where.trim() == ""
		|| where == "undefined") {
		finalUrl =  url+"?sessionId="+sessionId;
	} else {
		finalUrl = url + "?where=" + where+"&sessionId="+sessionId;
	}
